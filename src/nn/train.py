...
def predict(model, test_data):
    ...
    all_log_probs = torch.cat(log_probs_batches, dim=0) # FIXME: does not currently work because batches have different seq length :(
    all_targets = torch.cat(targets_batches, dim=0)     # Solution: in collate function optionally set a fixed seq length (for test data only)
